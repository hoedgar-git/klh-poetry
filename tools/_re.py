...
for ts,kind,slug in jobs:
    ...
    old=json.load(open(f"data/flows/{slug}.json")) if os.path.exists(f"data/flows/{slug}.json") else {}
    real="images/flow/"+slug
    if os.path.exists(real): shutil.rmtree(real)
    blocks=E.parse_docx(tmp)
    flow=E.build_flow(blocks,kind,"flow/"+slug,real)
...
# verify summary
def summ(slug):
    r=json.load(open(f"data/flows/{slug}.json")); print("==",slug,r["title"])
    for b in r["flow"]:
        ty=b["type"]
        if ty=="stanza": print("  ST"+("#" if b.get("num") else "")+" |", " / ".join(b["zh"])[:60])
        elif ty=="note": print("  NOTE("+str(len(b["points"]))+" pts) |", b["points"][0]["zh"][:50])
        elif ty=="preface": print("  PREFACE |", b["zh"][:60])
        elif ty=="image": print("  IMG", (b["caption_zh"][:40] or "(no cap)"))
        elif ty=="yt": print("  YT", len(b["urls"]),"links")
        elif ty=="title": print("  TITLE", b["zh"][:40])
        elif ty=="byline": print("  BY")
for s in ["p_5e319b47","p_c2ae755f","p_5cea581e","hdoc61"]: summ(s)
